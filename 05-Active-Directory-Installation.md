...
**Step 6:** Once the role installation finishes, a yellow warning flag will appear at the top of the Server Manager. Click it and select **Promote this server to a domain controller**.
...
> **Technical Explanation:** We select "Add a new forest" because this is the very first Domain Controller in our environment. Using a `.local` or an internal-only top-level domain is a common practice for isolated lab environments to prevent DNS conflicts with public internet domains.
...
> **Technical Explanation:** The DNS delegation warning is completely normal when creating the first DNS server in a new root forest, as there is no parent DNS zone to delegate authority from.
...
**Step 11: Prerequisites Check & Install**
